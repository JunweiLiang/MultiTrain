...
def train_epoch(
    ...
    if cfg.MODEL.USE_MULTI_HEAD:
        loss_fun = losses.MultiHeadLoss(cfg)
    else:
    ...
    if cfg.MODEL.USE_INF_LOSS:
        inf_loss_func = losses.INFLoss(cfg)
    ...
    for cur_iter, (inputs, labels, _, meta) in tqdm(enumerate(train_loader),
        ...
        # inputs are video frames, and meta includes bounding boxes for detection
        ...
        # for gradient accumulation, if num_batch=1 this is the same as normal
        # https://pytorch.org/docs/stable/notes/amp_examples.html#gradient-accumulation

        loss = loss / cfg.TRAIN.GRAD_ACCU_NUM_BATCH
        scaler.scale(loss).backward()

        if (cur_iter + 1) % cfg.TRAIN.GRAD_ACCU_NUM_BATCH == 0:
            # Unscales the gradients of optimizer's assigned params in-place to do clipping
...
def save_val_data(epoch, name, data, cfg):
    save_file = os.path.join(cfg.OUTPUT_DIR, "val_record.pkl")

    if os.path.exists(save_file):
        # read from the file first
        with open(save_file, "rb") as f:
            val_record = pickle.load(f)
    else:
        val_record = {}

    # a dictionaries of name -> epoch -> data
    if name not in val_record:
        val_record[name] = {}
    val_record[name][int(epoch)] = data

    with open(save_file, "wb") as f:
        pickle.dump(val_record, f)



def read_val_data(cfg):
    save_file = os.path.join(cfg.OUTPUT_DIR, "val_record.pkl")
    ...
    if os.path.exists(save_file):
        ...
        with open(save_file, "rb") as f:
            val_record = pickle.load(f)
        return {name: sorted([(epoch, val_record[name][epoch]) for epoch in val_record[name]], key=lambda x:x[0])
                for name in val_record}
    else:
        return {}
...
def train(cfg):
    ...
    for cur_epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCH):
        ...
        if is_eval_epoch:
            ...
            if du.is_master_proc():
                ...
                save_val_data(cur_epoch+1, "val_mAP/top5-err", mAP, cfg)
    ...
    if du.is_master_proc():
        val_record = read_val_data(cfg)
        print("val record: %s" % val_record)
