    ...
    if convert_from_caffe2:
        ...
        if inflation:
            ...
            for k, v in pre_train_dict.items():
                if k in replace_name_dict:
                    logger.info("replaced pretrain weight %s to %s" % (
                        k, replace_name_dict[k]))
                    k = replace_name_dict[k]
        ...
        if "epoch" in checkpoint.keys() and not epoch_reset:
            ...
            if not no_load_opt_state:
                # these will result in extra GPU memory when resuming from
                # other checkpoint and might lead to OOM (for CUDA 11.6)
                if optimizer:
...
def load_train_checkpoint(cfg, model, optimizer, scaler=None):
    ...
    if cfg.TRAIN.AUTO_RESUME and has_checkpoint(cfg.OUTPUT_DIR):
        ...
        checkpoint_epoch = load_checkpoint(
            ...
            no_load_opt_state=cfg.TRAIN.NO_LOAD_OPT_STATE,  # this could save some GPU memory spike
        )
        start_epoch = checkpoint_epoch + 1
    elif cfg.TRAIN.CHECKPOINT_FILE_PATH != "":
        logger.info("Load from given checkpoint file.")
        # check if we want to replace weight names
        replace_old_names = cfg.TRAIN.CHECKPOINT_REPLACE_NAME_OLD
        replace_new_names = cfg.TRAIN.CHECKPOINT_REPLACE_NAME_NEW
        assert len(replace_new_names) == len(replace_old_names), "checkpoint replace names not matched!"
        replace_name_dict = {}
        for o, n in zip(replace_old_names, replace_new_names):
            replace_name_dict[o] = n
        checkpoint_epoch = load_checkpoint(
            cfg.TRAIN.CHECKPOINT_FILE_PATH,
            model,
            cfg.NUM_GPUS > 1,
            ...
            replace_name_dict=replace_name_dict,
        )
