...
_C.TRAIN.EVAL_AND_EXIT = False  # eval first and then exit without training
...
# if set, replace checkpoint's param name
_C.TRAIN.CHECKPOINT_REPLACE_NAME_OLD = []  # list of names to be replaced from loaded checkpint
_C.TRAIN.CHECKPOINT_REPLACE_NAME_NEW = []

# If True, use FP16 for activations
_C.TRAIN.MIXED_PRECISION = False

# if true, will gather prediction and label before computing loss
# for loss that need the whole batch info like the INF
...
# perform gradient accumulation if num_batch > 1
# so if batch_size is 60, num_batch=2, effective batch_size will be 120
# https://pytorch.org/docs/stable/notes/amp_examples.html#gradient-accumulation
_C.TRAIN.GRAD_ACCU_NUM_BATCH = 1

# if you met OOM when resuming from last checkpoint,
# use this to ignore the optimizer states
_C.TRAIN.NO_LOAD_OPT_STATE = False
...
# use informative loss on the representation embedding
_C.MODEL.USE_INF_LOSS = False
_C.MODEL.INF_LOSS_WEIGHT = 0.1
_C.MODEL.USE_INF_EXPANDER = False
_C.MODEL.USE_INF_LEARN_WEIGHT = False
_C.MODEL.INF_STD_WEIGHT = 25.0
_C.MODEL.INF_COVAR_WEIGHT = 1.0

# moco copy the heads
_C.MODEL.MULTI_USE_MOCO = False
_C.MODEL.MULTI_MOCO_MOMENTUM = 0.99


# use mtl learned weight between dataset heads
_C.MODEL.MULTI_USE_MTL_WEIGHT = False

# for video-text model, we could ignore loading the text part if video features
# are ready
_C.MODEL.LOAD_VISUAL = True

# whether to ignore the classification head and extract features
_C.MODEL.GET_FEATURE = False
...
# if True, like MViT V2, will use relative positional embedding instead
_C.MVIT.REL_POS_EMBED = False
_C.MVIT.USE_SYM_REL = False  # use symmetric relative position embedding, so L length not 2L-1 Length
_C.MVIT.NO_EINSUM = False
_C.MVIT.REL_POS_ZERO_INIT = False
_C.MVIT.USE_SKEW = False
...
# this is not used
_C.MVIT.fixed_attn_scale = False  # scale * q

# --- MeMViT
_C.MVIT.USE_MEM = False
_C.MVIT.MEM_COMPRESS_FACTOR = [4, 2, 2]
_C.MVIT.MAX_MEM_LEN = 2
