...
def pack_mem_clips(frames, num_clip):
    # given frames, evenly devide the frames into num clips
    clip_length = int(frames.shape[1] / num_clip)
    # frames are [C, T, H, W]
    clips = [frames[:, i*clip_length:(i+1)*clip_length, :, :] for i in range(num_clip)]
    return clips
