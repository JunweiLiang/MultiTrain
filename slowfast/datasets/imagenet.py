...
class Imagenet(torch.utils.data.Dataset):
    ...
    def _load_imdb(self):
        ...
        """
        # junwei: change to a file list
        # (base) junweil@VM-252-202-centos:/youtu_pedestrian_detection/junweil/
        #imagenet/imagenet21k_resized$ find train -name "*.JPEG" > train.lst;
        #find val -name "*.JPEG" > val.lst

        file_lst_path = os.path.join(
            self.cfg.DATA.PATH_TO_PRELOAD_IMDB, f"{self.mode}.lst"
        )

        folders_to_imgs = defaultdict(list)
        with open(file_lst_path, "r") as f:
            for line in f:
                folder_name, file_name = line.strip().split("/")[-2:]
                file_path = os.path.join(self.data_path, self.mode, folder_name, file_name)

                folders_to_imgs[folder_name].append(file_path)

        self._class_ids = sorted(
            f for f in folders_to_imgs.keys() if re.match(r"^n[0-9]+$", f)
        )
        ...
        for class_id in self._class_ids:
            ...
            for file_path in folders_to_imgs[class_id]:
                self._imdb.append({"im_path": file_path, "class": cont_id})
    ...
    # to be compatible to other video datasets
    @property
    def num_videos(self):
        """
        Returns:
