...
def get_start_end_idx(
    ...
    if clip_idx == -1:
        ...
        # make sure the start_idx has at least one clip left
...
def decord_decode(
    ...
    sampling_rate,
    num_frames,
    clip_idx,  # temporal sample index
    num_clips=10,
    target_fps=30,
    use_offset=False,
    extra_num_clip=0,  # sample (extra_num_clip + 1) * sampling_rate * num_frames frames
):
    """
    Convert the video from its original fps to the target_fps. If the video
    support selective decoding (contain decoding information in the video head),
    ...
    start_idx, end_idx = get_start_end_idx(
        frames_length,
        (extra_num_clip + 1) * sampling_rate * num_frames / target_fps * fps,  # total frame for this video to sample
        clip_idx,
        num_clips,
        use_offset=use_offset,
    )
...
def decode(
    ...
    assert extra_num_clip == 0 or backend == "decord"
    try:
        if backend == "pyav":
    ...
    clip_sz = (extra_num_clip + 1) * sampling_rate * num_frames / target_fps * fps
    start_idx, end_idx = get_start_end_idx(
        frames.shape[0],
        clip_sz,
    ...
    frames = temporal_sampling(frames, start_idx, end_idx, int(num_frames * (extra_num_clip + 1)))
    return frames
